...
static void browser_apply_sort (menu_t *menu) {
    ...
    if (menu->browser.playlist) {
        browser_grid_reset_meta_index(menu);
        browser_grid_clear();
    }
...
static void browser_restore_playlist_overrides(menu_t *menu) {
    ...
    if (playlist_override.grid_view_applied) {
        browser_grid_set_enabled(playlist_override.saved_grid_view_enabled);
    }
    browser_grid_set_override(-1);
    browser_grid_clear();
...
static void browser_apply_playlist_overrides(menu_t *menu, const playlist_props_t *props) {
    ...
    playlist_override.saved_grid_view_enabled = browser_grid_get_enabled();
    ...
    if (want_grid_view) {
        browser_grid_set_enabled(props->grid_view == 1);
...
/* Search implementation extracted to browser_search.c */
...
static void process (menu_t *menu) {
    ...
    bool use_playlist_grid = browser_grid_is_enabled(menu);
    ...
    if (menu->actions.toggle_view && menu->browser.playlist && menu->browser.picker == BROWSER_PICKER_NONE) {
        bool currently_grid = browser_grid_is_enabled(menu);
        browser_grid_set_override(currently_grid ? 0 : 1);
        browser_grid_clear();
    ...
    browser_grid_prepare(menu, moved_selection);
...
static void draw (menu_t *menu, surface_t *d) {
    ...
    if (browser_search_is_active()) {
        browser_search_draw(menu);
    }

    ui_components_set_file_list_top_inset(0);

    if (!browser_search_is_active() && use_playlist_grid) {
        browser_grid_draw(menu);
    } else if (!browser_search_is_active()) {
...
void view_browser_display (menu_t *menu, surface_t *display) {
    ...
    browser_grid_prewarm_tick(menu);
}
